...
public class MSPageHelper {
    ...
    public static void main(String[] args) {
        String query = "selecct * from (select * from user) t";
        getLimitString(query,10,1);
    }
    ...
    private static int getLastOrderInsertPoint(String querySelect) {
        ...
        if (!isBracketCanPartnership(querySelect.substring(orderIndex, querySelect.length()))) {
            throw new RuntimeException("MS SQL 分页必须要有Order by 语句!");
        }
        return orderIndex;
    }
     ...
     */
    private static String getLineSql(String querySelect) {
